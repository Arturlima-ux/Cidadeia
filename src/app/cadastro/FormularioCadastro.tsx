...
type FormState = {
  ...
  site: string;
  // Pedido de proposta que originou este cadastro (vem de /proposta).
  propostaId: string;
};

/** O que /proposta já sabe e o cadastro não precisa perguntar de novo. */
export type InicialDoPedido = {
  propostaId: string;
  municipio: string;
  uf: string;
  nome: string;
...
export default function FormularioCadastro({ inicial }: { inicial: InicialDoPedido | null }) {
  const [form, setForm] = useState<FormState>(
    inicial
      ? {
          ...ESTADO_INICIAL,
          nomePrefeitura: `Prefeitura Municipal de ${inicial.municipio}`,
          estado: inicial.uf,
          municipio: inicial.municipio,
          nomeResponsavel: inicial.nome,
          emailResponsavel: inicial.email,
          propostaId: inicial.propostaId,
        }
      : ESTADO_INICIAL
  );
