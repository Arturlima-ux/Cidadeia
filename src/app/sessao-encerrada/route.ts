...
export async function GET(request: Request) {
  ...
  // "sessão expirada" seria mentira para quem nunca teve conta.
  const url = new URL(request.url);
  // Saindo da demo, o destino é /demo/fim: a home devolveria a pessoa à
  // mesma página que ela já tinha visto antes de entrar.
  const destino = new URL(url.searchParams.get("demo") ? "/demo/fim" : "/login?sessao=expirada", request.url);
